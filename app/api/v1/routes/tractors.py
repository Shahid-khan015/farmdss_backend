...
def list_tractors(
    ...
    effective_is_library = is_library
    if filter_type == "custom":
        effective_is_library = False
    elif filter_type == "library":
        effective_is_library = True
...
@router.get("/{tractor_id}/compatible-implements", response_model=List[ImplementRead])
def list_compatible_implements(
    tractor_id: uuid.UUID,
    _: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    return get_compatible_implements(tractor_id, db)
